...
describe("solana-sky-trade", () => {
  ...
  const fakemintAccount = new anchor.web3.PublicKey("DiW5MWFjPR3AeVd28ChEhsGb96efhHwst9eYwy8YdWEf");
  ...
  it("should successfully mint an nft", async () => {
    ...
    let dateNow = new Date("2024-07-26T19:30:12.738Z").toISOString(); //'2024-08-26T19:25:12.738Z'
    console.log({ dateNow });
    ...
    let test1 = await program.methods
    .mintRentalToken(
      landAssetId,
      dateNow,
      bump,
    ...
    //console.log({txsize:getTxSize(transactionV0, centralizedAccount.publicKey)});
     ...
     const txId = await provider.connection.sendTransaction(transactionV0).catch(e=>{console.log(e); return ""});
  ...
  it("should fail to mint an nft for invalid mins!=(00,30)", async () => {
    ...
    .catch((e:SendTransactionError)=>{
      ...
      let expectedString="Error Message: Provided minutes in the time should be 00 or 30."
  ...
  it("should fail as time is too far in the future", async () => {
    ...
    //console.log({txsize:getTxSize(transactionV0, centralizedAccount.publicKey)});
      ...
      .catch((e:SendTransactionError)=>{
      console.log(e.logs)
      let expectedString="Error Message: Provided time shouldnt be more than 3 month in future."
      let actualString=e.logs[(e.logs.length-2-1)] as string
      let ans=actualString.includes(expectedString)
      console.log({ans})
      assert.equal(ans,true);
